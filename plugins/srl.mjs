...
function checkScriptReadiness(files, cwd) {
  ...
  for (const f of files) {
    ...
    if (content.includes("SetCursorPos") || content.includes("mouse_event") || content.includes("SendInput")) {
      issues.push({ file: f, issue: "Uses physical mouse API — must use SMART-simulated Mouse.Move()/Mouse.Click() instead" });
    }
  }
  return issues;
}
...
- **S-Arch** — Script architecture design (state machines, task flow, anti-ban strategy)
...
- **S-Review** — Script review (correctness + anti-ban + performance)
- **S-Correctness** — Logic validation, edge cases, state machine analysis
- **S-Sentry** — Anti-ban/anti-detection audit
- **S-Perf** — Script performance optimization
- **S-Skeptic** — Devil's advocate, stress-testing the design
...
Use these agents by invoking S-AgentName when working on OSRS Simba scripts.`);
  ...
  tool: {
    "check-script-readiness": tool({
      description:
        "Check a set of .simba files for script readiness. Scans for TODOs, FIXMEs, HACKs, empty stubs, missing anti-ban, hardcoded waits, missing includes, infinite loops, and physical mouse usage. Run before considering a script complete.",
      args: {
        files: tool.schema
...
// ---- State machine ----
type
  TScriptState = (STATE_FIND_TASK, STATE_EXECUTING, STATE_BANKING, STATE_IDLE);
...
begin
  // TODO: implement the main task logic for this ${scriptType} script
  // This is called each loop while in STATE_EXECUTING
end;

procedure BankProcedure();
begin
  // TODO: implement banking logic
  // Called while in STATE_BANKING
end;
